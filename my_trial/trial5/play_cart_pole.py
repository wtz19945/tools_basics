...
class CartPole(PipelineEnv):
    ...
    def reset(self, rng: jax.Array) -> State:
        ...
        theta_init = jax.random.uniform(theta_key, (1,), minval=-.5, maxval=.5)[0]
        qinit = jax.numpy.array([0.0, theta_init])
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        outside_x = jax.numpy.abs(x_position) > 1.5
        outside_th = jax.numpy.abs(rot_position) > jax.numpy.pi / 2
        done = outside_x
...
train_fn = functools.partial(
    ppo.train,
    num_timesteps=0,  # Increase training time
    num_evals=10,
    episode_length=200,
    num_envs=64,
    num_eval_envs=4,
    ...
    num_minibatches=4,
    unroll_length=20,
    num_updates_per_batch=8,  # More updates per batch for faster learning
    normalize_observations=True,
    discounting=0.99,  # Slightly higher discounting
    learning_rate=1e-4,  # Lower learning rate for larger starting angles
    entropy_cost=1e-3,  # Slightly lower entropy cost for better exploration
