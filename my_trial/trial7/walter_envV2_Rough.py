...
class WalterEnv(PipelineEnv):
    ...
    def __init__(self, 
        ...
        filepath = os.path.join(os.path.dirname(__file__), ROOT_PATH/'scene_roughterrain.xml')
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        m_tsh = action[1:8:4] * self._action_scale
        m_tw1 = action[2:8:4] * self._action_scale
        m_tw2 = action[3:8:4] * self._action_scale
        
        m_hth = self._default_pose[9::4] + action[8::4] * self._action_scale
        m_hsh = action[9::4] * self._action_scale
        m_hw1 = action[10::4] * self._action_scale
        m_hw2 = action[11::4] * self._action_scale
        
        motor_targets = jnp.concatenate([m_tth, m_tsh, m_tw1, m_tw2, m_hth, m_hsh, m_hw1, m_hw2])
        ...
        done |= (pipeline_state.site_xpos[self._site_id_head].ravel()[2] < 0.1)
        rewards = {
            'tracking_lin_vel': (
    ...
    def _reward_tracking_lin_vel(self, command: jax.Array, x: Transform, xd: Motion, pipeline_state: base.State) -> jax.Array:
        ...
        abs_error = jnp.sum(jnp.abs(torso_vel[:2] - command[:2]) + jnp.abs(head_vel[:2] - command[:2]))
