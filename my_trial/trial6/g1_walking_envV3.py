    ...
    def get_default_rewards_config():
        default_config = config_dict.ConfigDict(
            dict(
                ...
                tracking_sigma_linvel=0.2,
                tracking_sigma_angvel=0.2,
                tracking_sigma_linvelz=0.05,
                tracking_sigma_angvelxy=0.02,
                tracking_sigma_baseheight=0.02,
                tracking_sigma_orientation=0.02,
                tracking_sigma_torques=1500,
                tracking_sigma_action_rate=50,
                tracking_sigma_stand_still=0.2,
                tracking_sigma_contact_force=0.05,
                tracking_sigma_joint_deviation_knee=0.2,
                tracking_sigma_joint_deviation_hip=0.2,
                tracking_sigma_dof_pos_limits=0.05,
                tracking_sigma_pose=0.3,
                tracking_sigma_feetairtime=0.2,
                tracking_sigma_slip=0.05,
                tracking_sigma_phase=0.2,
...
class G1Env(PipelineEnv):
    ...
    def __init__(self, 
        ...
        ROOT_PATH = epath.Path('/home/orl/Tianze/mujoco_work/mp_source/mujoco_playground/mujoco_playground/_src/locomotion/g1/xmls')
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        # physics step
        action_new = jnp.concatenate([action[:-14], jnp.zeros(14)])
        action = action_new
    ...
    def _reward_contact_force(self, data: base.State) -> jax.Array:
        ...
        cost = jnp.exp(-jnp.abs(cost) / self.sys_config.rewards.tracking_sigma_contact_force)
        return cost
    ...
    def _reward_tracking_lin_vel(self, command: jax.Array, data: base.State) -> jax.Array:
        ...
        lin_vel_reward = jnp.exp(-lin_vel_error / self.sys_config.rewards.tracking_sigma_linvel)
    ...
    def _reward_tracking_ang_vel(self, command: jax.Array, data: base.State) -> jax.Array:
        ...
        ang_vel_reward = jnp.exp(-ang_vel_error / self.sys_config.rewards.tracking_sigma_angvel)
    ...
    def _reward_lin_vel_z(self, data: base.State) -> jax.Array:
        ...
        pel_dz = data.sensordata[self._pelvis_global_linvel_vector_adr][2]
        tor_dz = data.sensordata[self._torso_global_linvel_vector_adr][2]
        reward = jnp.exp(-(jnp.abs(pel_dz) + jnp.abs(tor_dz)) / self.sys_config.rewards.tracking_sigma_linvelz)
        return reward
    
    def _reward_ang_vel_xy(self, data: base.State) -> jax.Array:
        """Reward for the base angular velocity in xy direction."""
        tor_angvel = data.sensordata[self._torso_global_angvel_vector_adr][:2]
        reward = jnp.exp(-jnp.sum(jnp.abs(tor_angvel)) / self.sys_config.rewards.tracking_sigma_angvelxy)
        return reward
    ...
    def _reward_base_height(self, data: base.State) -> jax.Array:
        ...
        base_height_error = jnp.abs(base_height - base_height_target)   
        reward = jnp.exp(-base_height_error / self.sys_config.rewards.tracking_sigma_baseheight)
        return reward
    ...
    def _reward_orientation(self, data: base.State) -> jax.Array:
        ...
        error = jnp.sum(jnp.square(torso_zaxis - jnp.array([0.073, 0.0, 1.0])))
        reward = jnp.exp(-error / self.sys_config.rewards.tracking_sigma_orientation)
        return reward
    
    def _reward_torques(self, torques: jax.Array) -> jax.Array:
        """Reward for the joint torques."""
        error = jnp.sum(jnp.abs(torques)/500)
        reward = jnp.exp(-error / self.sys_config.rewards.tracking_sigma_torques)
        return reward
    
    def _reward_action_rate(self, action: jax.Array, last_action: jax.Array) -> jax.Array:
        """Reward for the action rate."""
        error = jnp.sum(jnp.abs(action - last_action))
        reward = jnp.exp(-error / self.sys_config.rewards.tracking_sigma_action_rate)
        return reward
    ...
    def _reward_stand_still(self, command: jax.Array, joint_angles: jax.Array) -> jax.Array:
        ...
        error = jnp.sum(jnp.square(joint_angles - self._default_pose))
        return jnp.exp(-error / self.sys_config.rewards.tracking_sigma_stand_still) * (
    ...
    def _reward_joint_deviation_knee(self, qpos: jax.Array) -> jax.Array:
        error = jnp.sum(
            jnp.square(qpos[self._knee_indices] - self._default_pose[self._knee_indices])
        )
        reward = jnp.exp(-error / self.sys_config.rewards.tracking_sigma_joint_deviation_knee)
        return reward
    ...
    def _reward_joint_deviation_hip(
        ...
        cost = jnp.sum(jnp.square(error) * weight)
        reward = jnp.exp(-cost / self.sys_config.rewards.tracking_sigma_joint_deviation_hip)
        # cost = jnp.sum(
    ...
    def _reward_pose(self, qpos: jax.Array) -> jax.Array:
        ...
        # error = jnp.sum(jnp.square(qpos - self._default_pose))
        error = jnp.sum(jnp.abs(qpos - self._default_pose)  * self._weights)
        reward = jnp.exp(-error / self.sys_config.rewards.tracking_sigma_pose)
        return reward
    ...
    def _reward_joint_pos_limits(self, qpos: jax.Array) -> jax.Array:
        ...
        reward = jnp.exp(-jnp.sum(jnp.abs(out_of_limits)) / self.sys_config.rewards.tracking_sigma_dof_pos_limits)
        return reward
    ...
    def _cost_feet_slip(self, data: base.State, contact: jax.Array, state_info: dict[str, Any]) -> jax.Array:
        ...
        reward = jnp.sum(jnp.linalg.norm(body_vel, axis=-1) * contact)
        reward = jnp.exp(-reward / self.sys_config.rewards.tracking_sigma_slip)
        return reward
    ...
    def _reward_feet_phase(self, data: base.State, phase: jax.Array, foot_height: jax.Array, command: jax.Array) -> jax.Array:
        ...
        rz = self.bezier_phase_ramp(x, 0.1) * foot_height
        error = jnp.sum(jnp.square(foot_z - rz))
        reward = jnp.exp(-error / self.sys_config.rewards.tracking_sigma_phase)
        return reward
